...
static bool has_return(AST *n) {
  ...
  if (n->left && has_return(n->left))
    return true;
  if (n->right && has_return(n->right))
    return true;
  if (n->next && has_return(n->next))
    return true;
...
void check_types(AST *n) {
  ...
  case NODE_BINOP: {
    ...
    if (left_expr_type == TYPE_VOID || right_expr_type == TYPE_VOID) {
      fprintf(stderr, "Type error: cannot use void value in an expression\n");
      type_check_error = true;
      n->info->eval_type = TYPE_ERROR;
      break;
    }
    ...
    if (strcmp(n->info->op, "==") == 0) {
      if (left_expr_type != right_expr_type) {
        fprintf(stderr,
                "Type error: '==' requires operands of the same type, got %s "
                "and %s\n",
      ...
      n->info->eval_type = TYPE_BOOL;
               ...
               strcmp(n->info->op, "*") == 0 || strcmp(n->info->op, "/") == 0 ||
               strcmp(n->info->op, "%") == 0) {
      if (left_expr_type != TYPE_INT || right_expr_type != TYPE_INT) {
        fprintf(stderr,
                "Type error: arithmetic operator '%s' requires integers, got "
                "%s and %s\n",
                n->info->op, type_to_string(left_expr_type),
                type_to_string(right_expr_type));
        type_check_error = true;
      ...
      if (left_expr_type != TYPE_BOOL || right_expr_type != TYPE_BOOL) {
        fprintf(stderr,
                "Type error: logical operator '%s' requires booleans, got %s "
                "and %s\n",
                n->info->op, type_to_string(left_expr_type),
                type_to_string(right_expr_type));
        type_check_error = true;
  ...
  case NODE_ASSIGN: {
    ...
    if (n->left->info && n->left->info->is_function) {
      fprintf(stderr, "Type error: cannot assign to function '%s'.\n",
              n->left->info->name);
      type_check_error = true;
      break;
    }
    ...
    if (id_type != expr_type) {
      fprintf(stderr,
              "Type error: assignment mismatch (expected %s but got %s)\n",
              type_to_string(id_type), type_to_string(expr_type));
      type_check_error = true;
    }
    ...
    if (!n->left) {
      ...
      if (n->info->eval_type != func_type) {
        fprintf(stderr, "Type error: function expects %s but returns %s\n",
                type_to_string(func_type), type_to_string(n->info->eval_type));
        type_check_error = true;
      }
    }
    break;
  ...
  case NODE_FUNCTION: {
    ...
    if (func_type != TYPE_VOID) {
      bool is_extern =
          (n->right && n->right->type == NODE_ID && n->right->info &&
           n->right->info->name && strcmp(n->right->info->name, "EXTERN") == 0);

      if (!is_extern && !has_return(n->right)) {
        fprintf(stderr,
                "Type error: function '%s' declared as %s but has no return "
                "statement.\n",
  ...
  case NODE_CALL: {
    ...
    while (formal_params != NULL && actual_params != NULL) {
      ...
      if (formal_params->param_type != actual_params->info->eval_type) {
        fprintf(stderr,
                ...
                n->info->name, type_to_string(formal_params->param_type),
    ...
    if (formal_params != NULL) {
      fprintf(stderr, "Error in call to %s: missing parameters.\n",
              n->info->name);
      type_check_error = true;
    }

    if (actual_params != NULL) {
      fprintf(stderr, "Error in call to %s: too many parameters.\n",
              n->info->name);
      type_check_error = true;
    }
