    ...
    case NODE_BINOP: {
        ...
        if (strcmp(n->info->op, "==") == 0) {
            if (left_expr_type != right_expr_type) {
                fprintf(stderr,
                        "Type error: '==' requires operands of the same type, "
                        "got %s "
                        "and %s\n",
            ...
            if (left_expr_type != TYPE_INT || right_expr_type != TYPE_INT) {
                fprintf(stderr,
                        "Type error: '%s' requires integer operands, got %s "
                        "and %s\n",
            ...
            if (left_expr_type != TYPE_INT || right_expr_type != TYPE_INT) {
                fprintf(stderr,
                        "Type error: arithmetic operator '%s' requires "
                        "integers, got "
                        "%s and %s\n",
                        n->info->op, type_to_string(left_expr_type),
                        type_to_string(right_expr_type));
            ...
            if (left_expr_type != TYPE_BOOL || right_expr_type != TYPE_BOOL) {
                fprintf(stderr,
                        "Type error: logical operator '%s' requires booleans, "
                        "got %s "
                        "and %s\n",
                        n->info->op, type_to_string(left_expr_type),
                        type_to_string(right_expr_type));
    ...
    case NODE_RETURN: {
        if (!n->left) {
            ...
            if (func_type != TYPE_VOID) {
                fprintf(stderr,
                        "Type error: function expects %s but returns void\n",
            ...
            if (n->info->eval_type != func_type) {
                fprintf(stderr,
                        "Type error: function expects %s but returns %s\n",
                        type_to_string(func_type),
    ...
    case NODE_FUNCTION: {
        ...
        if (func_type != TYPE_VOID) {
            ...
            if (!is_extern && !has_return(n->right)) {
                fprintf(stderr,
                        "Type error: function '%s' declared as %s but has no "
                        "return "
                        "statement.\n",
                        (n->info && n->info->name) ? n->info->name
                                                   : "(unknown)",
                        type_to_string(func_type));
                type_check_error = true;
            }
    ...
    case NODE_CALL: {
        ...
        while (formal_params != NULL && actual_params != NULL) {
            ...
            if (formal_params->param_type != actual_params->info->eval_type) {
                fprintf(stderr,
                        "Type error in call to %s: expected type %s but got "
                        "type %s.\n",
                        n->info->name,
