...
static char *gen_expr(AST *n, FILE *out) {
    ...
    switch (n->type) {
        ...
        case NODE_CALL: {
            ...
            char *res = new_temp();
            emit(out, "%s = call %s, %d", res, n->info->name ? n->info->name : "(unknown)", argnum);
            return res;
        }
        default:
            if (n->left) {
...
static void gen_stmt(AST *n, FILE *out) {
    ...
    switch (n->type) {
        ...
        case NODE_ASSIGN: {
            char *lhs = NULL;
            if (n->left && n->left->type == NODE_ID && n->left->info && n->left->info->name) {
        ...
        case NODE_FUNCTION: {
            const char *fname = n->info && n->info->name ? n->info->name : "(anon)";
            ...
            if (n->left) {
                ...
                int i = 0;
                while (p) {
                    emit(out, "  # param %d -> %s", i++, p->info && p->info->name ? p->info->name : "(p)");
                    p = p->next;
                }
            }
        ...
        case NODE_BLOCK: {
            if (n->left) gen_stmt_list(n->left, out);
            if (n->right) gen_stmt_list(n->right, out);
            break;
        }
        ...
        default:
            if (n->left) gen_stmt(n->left, out);
            if (n->right) gen_stmt(n->right, out);
            break;
    }

    if (n->next) gen_stmt(n->next, out);
}
