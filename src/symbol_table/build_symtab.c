...
static void build_block(AST *blockNode, SymTab *parent, FILE *stream) {
  ...
  if (blockNode->left)
    build_symtab_list(blockNode->left, target, stream);
  if (blockNode->right)
    build_symtab_list(blockNode->right, target, stream);
...
TypeInfo build_symtab(AST *n, SymTab *st, FILE *stream) {
  ...
  case NODE_PROG:
    if (n->left)
      build_symtab_list(n->left, st, stream);
    if (n->right)
      build_symtab_list(n->right, st, stream);
    break;
  ...
  case NODE_FUNCTION: {
    if (symtab_scope(st, n->info->name) != TYPE_ERROR) {
      fprintf(stderr,
              "Error: A declaration (variable or function) with the name '%s' "
    ...
    if (n->left)
      build_symtab_list(n->left, fnScope, stream);
    if (n->right)
      build_symtab(n->right, fnScope, stream);
  ...
  case NODE_IF:
    if (n->left)
      build_symtab(n->left, st, stream);

    if (n->right)
      build_symtab(n->right, st, stream);
